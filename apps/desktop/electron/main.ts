...
// Every primary-backend start waits for the last invalidated child to exit.
// Some recovery paths reset synchronously and reload immediately, so the wait
// must live outside the caller-specific teardown helper.
let primaryBackendTeardownPromise: Promise<void> = Promise.resolve()
...
function resetHermesConnection({ soft = false } = {}) {
  ...
  const dying = backendConnectionState.invalidate()
  stopBackendChild(dying)

  if (dying) {
    primaryBackendTeardownPromise = waitForBackendExit(dying).catch(error => {
      rememberLog(
        `Primary Hermes backend teardown wait failed: ${error instanceof Error ? error.message : String(error)}`
      )
    })
  }
