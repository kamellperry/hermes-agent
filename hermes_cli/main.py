...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    r1 = _run_npm_install_deterministic(
        npm,
        npm_cwd,
        # Dashboard builds need dev dependencies such as TypeScript and Vite,
        # even when Hermes was launched with NODE_ENV=production.
...
def _git_cmd_for_update() -> list[str]:
    ...
    Resolve an absolute executable when possible so sparse macOS GUI/TUI
    environments and accidentally quoted PATH entries cannot make update try
    to execute a literal ``git\"`` token.
    """
    git_exe = shutil.which("git")
    if not git_exe and sys.platform == "darwin" and Path("/usr/bin/git").exists():
        git_exe = "/usr/bin/git"
    git_exe = (git_exe or "git").strip().strip('"').strip("'")
    git_cmd = [git_exe]
    if sys.platform == "win32":
        git_cmd.extend(["-c", "windows.appendAtomically=false"])
    return git_cmd
...
def _resolve_update_branch(args) -> str:
    ...
    An explicit ``--branch`` always wins.  Without one, preserve the durable
    ``patched-main`` fleet checkout instead of silently switching it to
    ``main`` and updating only the upstream mirror.  All other checkouts keep
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip() == "patched-main":
