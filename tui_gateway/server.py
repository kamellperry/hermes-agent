        ...
        try:
            ...
            try:
                ...
                if isinstance(resume_overrides, dict) and resume_overrides:
                    ...
                    elif current.get("reasoning_config_override") is not None:
                        # Compatibility with sessions created by patched-main
                        # before upstream standardized the create_* field.
                        kw["reasoning_config_override"] = current.get(
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "model_override": None,
            # Legacy alias retained for restored patched-main sessions. New
            # writes also populate create_reasoning_override.
...
def _(rid, params: dict) -> dict:
    ...
    if key == "reasoning":
        try:
            ...
            persist_global = scope == "global" or any(
                token in {"--global", "--save"} for token in tokens
            )
            tokens = [
                token for token in tokens if token not in {"--global", "--save", "--session"}
            ]
            ...
            if session is not None and not persist_global and session.get("running"):
                return _err(
                    rid,
                    4009,
                    "session busy — /interrupt the current turn before changing reasoning",
                )
            if persist_global or session is None:
                _write_config_key("agent.reasoning_effort", arg)
                if session is not None:
                    session.pop("create_reasoning_override", None)
                    session.pop("reasoning_config_override", None)
            else:
            ...
            return _ok(
                rid,
                {
                    "key": key,
                    "value": arg,
                    "scope": "global" if persist_global or session is None else "session",
                },
            )
        except Exception as e:
            return _err(rid, 5001, str(e))
    ...
    if key == "reasoning":
        ...
        if session is not None:
            if isinstance(session.get("create_reasoning_override"), dict):
                ...
                elif isinstance(session.get("reasoning_config_override"), dict):
                    reasoning_config = session.get("reasoning_config_override")
